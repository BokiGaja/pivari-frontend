...
const ArticlePreview = ({ article }) => {
  ...
  return (
    <div className="flex flex-col w-3/4 h-[450px] bg-guinessBlack transition-transform transform-gpu hover:scale-105 cursor-pointer transition-all duration-500 rounded-3xl">
      <ImageWithFade url={article?.cover_image?.data?.attributes?.url ? backgroundImageUrl : pivariLogo} />
      <div className="flex flex-col text-white z-10 transition-transform hover:text-xl p-5">
