      ...
      <div className="flex flex-col items-center px-20">
        {members?.map((member, index) => (
          <React.Fragment key={member?.createdAt}>
            ...
            <div
              className="flex flex-col justify-center tems-stretch border border-maltYellow rounded-3xl mt-5  bg-guinessBlack w-6/12 hover:scale-105 hover:cursor-pointer transition-all duration-[500ms]"
              onClick={() => navigate(`/members/${member?.name?.replace(' ', '-')}`)}
            >
